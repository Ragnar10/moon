...
const WithMetamask = () => {
    ...
    const metaBtn = isMobileDevice() && !wallet
        ?  <a
            href = { `https://metamask.app.link/dapp/${process.env.REACT_APP_METAMASK_API_PATH}/affiliate/${influencer}?influencer=${influencer}&user_tg=${user.telegram}&user_tw=${user.twitter}&user_token=${user.token}&meta=true` }
